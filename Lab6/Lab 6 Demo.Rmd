...
summary(earnings_df$age_recode)

new_pred_IV <- tibble(age_recode = rep(20:65)) %>%
        mutate(
          edu = mean(earnings_df$edu),
          female = mean(earnings_df$female),
          black = mean(earnings_df$black),
          other = mean(earnings_df$other))


# Examine the df
head(new_pred_IV, 20) %>% kbl("html") %>% kable_classic_2(full_width = F)
```

```{r}
# use `predict` to predict the Y
new_predicted_earning <- predict(m5,
                                 new_pred_IV,
                                 interval = "confidence",
                                 level = 0.95)

# bind the columns
new_pred_result <- cbind(new_pred_IV, new_predicted_earning)

# check df
head(new_pred_result, 20) %>% kbl("html") %>% kable_classic_2(full_width = F)
```
...
new_pred_result %>% 
  ...
  geom_line(aes(x = age_recode, y = fit)) +                              # group linetype by age
...
earnings_df <- earnings_df %>% mutate(
  age_sq = age_recode*age_recode 
)

m6 <- lm(earn ~ age_recode + edu + female + black + other + edu*female + age_sq, data = earnings_df)

# Null hypothesis: H0 = m5 - m6 = 0
# Alternative hypothesis: Ha = m5 - m6 =/=0

anova(m5, m6) # The additional predictors don't really add anything, p> 0.05
```
