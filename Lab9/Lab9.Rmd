...
```{r}
pred_IV <- tibble(eduy = seq(0, 25, 1)) %>%         #create df with key IV
  mutate(age = mean(support_df$age), na.rm = T,
black = mean(support_df$black), na.rm = T,
female = mean(support_df$female), na.rm = T)
```

```{r}
# use `predict` to predict the Y
# the model you are using, the df you use, setting CI
predicted_support <- predict(m4, pred_IV, interval = "confidence", level = 0.95)

# bind the columns
pred_result <- cbind(pred_IV, predicted_support)
```
...
pred_result %>% 
  ...
  labs(x = "Eduy",
       y = "Predicted Support") +
  ggtitle("Predicted Support by Eduy")
```
