...
covid_data <- covid_data %>%
  mutate(vaccine_counterf = ifelse(vaccine == 1, 0, 1),
         risk_counterf = rescale(0 - 7*vaccine_counterf - 2*info + e, to = c(0,100)))
...
```{r}
mean(treat$covid_risk) - mean(cf_treat$risk_counterf)
```


(c) Treatment Effect on the Control
```{r}
mean(control$covid_risk) - mean(cf_control$risk_counterf)
```

  
(d) Selection Bias
```{r}
mean(cf_treat$risk_counterf) - mean(control$covid_risk)
```
