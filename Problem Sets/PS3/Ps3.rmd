...
library(pacman)
p_load(tidyverse, kableExtra, sandwich, scales)
...
```{r}
treat <- covid_data %>% filter(covid_data$vaccine == "1") %>% sum(covid_data$`Risk Factor`)/nrow(covid_data)
control <- covid_data %>% filter(covid_data$vaccine == "0") %>% sum(covid_data$`Risk Factor`)/nrow(covid_data)
cf_treat <- covid_data %>% filter(covid_data$vaccine_counterf == "1") %>% sum(covid_data$"risk_counterf")/nrow(covid_data)
cf_control <- covid_data %>% filter(covid_data$vaccine_counterf == "0") %>% sum(covid_data$"risk_counterf")/nrow(covid_data)
treat
control
cf_treat
cf_control
