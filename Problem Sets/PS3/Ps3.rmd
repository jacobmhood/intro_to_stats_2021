...
library(pacman)
p_load(tidyverse, kableExtra, sandwich, scales, stargazer, arules)
...
We're interested in the effect of an area having a vaccination center on that area's rate of COVID infections. However, there is one big problem with this: it's not possible to observe effects of both having and not having a vaccination center in the same area at the same time. This is the biggest problem we face when trying to do causal inference, or when trying to claim that "X" variable affects "Y" variable in a certain way. Constructing a counterfactual framework for this study would help us be able to make stronger causal claims. Basically, we can construct extra variables that  simulate the other potential outcome we can't directly observe in our data: what would happen to infection rates if an area that received the treatment (having a vaccination center) didn't receive the treatment, for instance. If we think that the quality of information might be an unobserved factor, then we have a problem of selection bias. This means that we can expect those who receive higher quality information will be more likely to want and ultimately receive vaccinations. People aren't randomly assigned to live in these different areas, and we can also expect that the quality of information people get about COVID is associated with where they live. Thus, there are important differences between those in the control group and those in the treatment group that we must consider in order to figure out the effect of vaccination centers on infection rates. 

A biased guess of the average treatment effect (ATE) is, that for areas having a vaccination center (treatment) compared to those that don't (control), they have a 25.5 unit lower rate of COVID infections. It's a biased and thus unreliable guess because 1) as discussed, it doesn't take into account both potential outcomes happening in the same area and 2) the quality of information is creating an issue of selection bias. Our counterfactual framework helps here. The treatment effect on the treated we calculate with the help of this framework (16.31 unit decrease in COVID infections) is not equal to the biased guess we calculated earlier (25.5). This shows that our hunch about selection bias in the data is true. If people were randomly assigned to these areas, these numbers would be equal, so information is playing an important role here. Also with this framework, we can calculate the effect of not having a vaccination center for the areas we originally observed in this condition: an increase of 17.6 units. Since we know quality of information residents receive is significant here, we can calculate the selection bias. We take the difference between the effect on infection rates with our new variable simulating what would happen if areas that have vaccination centers didn't have vaccination centers, and the effect on infection rates we actually observe in those areas without vaccination centers, giving us -9.19. This gives us confirmation that quality of information is playing a role both in which areas have vaccination centers and those areas COVID infection rates. 
...
stargazer(lpm1, logit1, type = "text", omit.stat = c("ser","f"))
```
...
In the logistic model, for a 1 unit increase in GRE score, the odds of being admitted increases by a factor of e^0.002, or 0.005. For a 1 unit increase in GPA, the odds of being admitted increases by a factor of e^0.777, or 2.18. For a 1 unit increase in school ranking (meaning schools with lower rankings), the odds of being admitted increases by a factor of e^-0.560, or -0.057. 
...
pred_IV <- tibble(gpa = rescale(admin_data$gpa, to =c(1,4)), rank = admin_data$rank) %>% mutate(gre = mean(admin_data$gre, na.rm = T))
...
# Plot
pred_result %>% 
  ggplot(aes(x = gpa, y = predict_admit, color = as.factor(rank))) + # Have to convert rank into discrete variable
  geom_line() +   # group linetype by gender
  labs(x = "GPA",
       y = "Predicted Log Odds of Admission",
       fill = "Rank") +
  ggtitle("Predicted Log Odds of Admission by GPA and School Rank") +
